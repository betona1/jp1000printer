...
- **인기도서 그리드**: 독서로 인기도서 TOP10 가로 스크롤을 세로 그리드로 CSS 주입 변환
- **OTA 업데이트**: GitHub 릴리즈 기반 자동 업데이트 확인 + APK 다운로드/설치
...
| 인기도서 TOP10 그리드 | ON | 독서로 인기도서 세로 그리드 변환 |
...
| `top_book_grid` | Boolean | true | 인기도서 그리드 표시 |
| `print_driver_hidden` | Boolean | false | 인쇄 드라이버 상태 숨김 |
| `setup_guide_dismissed` | Boolean | false | 초기 설정 가이드 닫힘 |
...
`onPageFinished`에서 조건부 CSS/JS를 주입합니다.

| 주입 | 조건 | 내용 |
|------|------|------|
| select 스타일 수정 | 모든 페이지 | WebView v83 wavy border 수정 |
| 인기도서 그리드 | SchoolSearch/PureScreen URL + 설정 ON | Swiper → flex-wrap 그리드 (기기별 컬럼 수 분기) |
| window.print 오버라이드 | 모든 페이지 | NativeBridge.print() 호출 |
| window.open 오버라이드 | 모든 페이지 | 새 창 대신 같은 WebView에서 이동 |

**인기도서 그리드 기기별 컬럼 수** (`BuildConfig.FLAVOR` 분기):

| 기기 | 세로모드 | 가로모드 | CSS orientation 참고 |
|------|----------|----------|---------------------|
| A40i (a40) | 3개 (30%) | 4개 (23%) | 정상 (가로형 디스플레이) |
| JY-P1000 (standard) | 3개 (30%) | 2개 (48%) | CSS orientation 뒤바뀜 (REVERSE_PORTRAIT) |
...
### A40i WebView/Chrome 버전과 인쇄 품질

A40i는 기본 WebView(v52)가 너무 오래되어 Chrome 113을 WebView provider로 사용합니다.

**Chrome 113 미설치 시 증상:**
- 웹페이지 렌더링 깨짐 (CSS3/ES6 미지원)
- `createPrintDocumentAdapter()`가 생성하는 PDF 해상도가 JY-P1000과 달라짐
- 인쇄 결과 확대/축소, 세로 길이 차이 발생

**원인:** WebView 버전에 따라 PDF 생성 시 DPI/레이아웃 계산이 다름.
- JY-P1000: WebView v83, density 142
- A40i: Chrome 76(구버전) density 160 → PDF page.width가 달라 scale 비율 차이

**해결:** Chrome 113 필수 설치 + WebView provider 설정
```bash
# 기존 Chrome 제거 (서명 불일치 방지)
adb shell pm uninstall com.android.chrome
# Chrome 113 설치
adb install -r chrome113.apk
# WebView provider 변경
adb shell settings put global webview_provider com.android.chrome
```

`setup_a40i.sh` Step 3에서 자동 처리됩니다.
`configure_print.sh` 부트 스크립트에서 재부팅 후에도 WebView provider 설정을 자동 복원합니다.

### GreenMango 원격제어 포커스 문제

Compose AlertDialog의 OutlinedTextField가 여러 개일 때 접근성 서비스가 첫 필드에만 포커스.
→ 비밀번호 변경 다이얼로그를 Android 네이티브 AlertDialog + EditText로 전환하여 해결.
