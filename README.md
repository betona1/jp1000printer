...
| 인쇄 폭 | 576px (72 bytes/row), 72mm | 576px (72 bytes/row), 72mm |
| 해상도 | 203 DPI | 203 DPI |
...
├── BootCompletedReceiver.kt # 부팅 시 PrintService 자동 재활성화
├── PowerScheduleManager.kt  # AlarmManager 기반 화면 on/off 스케줄
...
| 이름 | 크기 | 용도 |
|------|------|------|
| 72mm x 150mm | 2835 x 5906 mils | 기본값 (라벨, 짧은 문서) |
| 72mm x 300mm | 2835 x 11811 mils | 중간 문서 |
| 72mm x 600mm | 2835 x 23622 mils | 긴 문서 |

## 관리자 설정

홈 화면 좌측 상단 설정 아이콘 → 비밀번호 입력 (기본: `1234`)

| 설정 | 기본값 | 설명 |
|------|--------|------|
| 학교주소 | `read365.edunet.net/SchoolSearch` | WebView 홈페이지 |
| 부팅 후 자동실행 | ON | init.rc 스크립트 기반 |
| 인기도서 TOP10 그리드 | ON | 독서로 인기도서 세로 그리드 변환 |
| 야간 절전모드 | OFF | 주간 활성시간 외 화면 끔 |
| 관리자 비밀번호 | 1234 | 네이티브 다이얼로그 (원격제어 호환) |
...
- `WRITE_SECURE_SETTINGS` 권한 없으면 앱에서 인쇄 드라이버 자동 활성화 불가 → 관리자에서 수동 숨김 가능
- GreenMango 원격제어 시 Compose TextField 포커스 문제 → 네이티브 EditText 사용
